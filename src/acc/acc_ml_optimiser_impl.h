...
void getAllSquaredDifferencesFine(
	...
	AccPtrFactory ptrFactory,
	int ibody,
	std::vector<AccPtrBundle > &bundleD2,int thread_id)
{
#ifdef TIMING
	// if (op.part_id == baseMLO->exp_my_first_part_id)
	if(thread_id==0)
...
#ifdef TIMING
	if(thread_id==0)
		baseMLO->timer.tic(baseMLO->TIMING_EXTRA2);
	baseMLO->timer.tic(baseMLO->TIMING_EXTRA2_T[thread_id]);
...
#ifdef TIMING
	if(thread_id==0)
		baseMLO->timer.toc(baseMLO->TIMING_EXTRA2);
	baseMLO->timer.toc(baseMLO->TIMING_EXTRA2_T[thread_id]);
...
void storeWeightedSums(OptimisationParamters &op, SamplingParameters &sp,
						...
						AccPtrFactory ptrFactory,
						int ibody,
						std::vector< AccPtrBundle > &bundleSWS,int thread_id)
{
#ifdef TIMING
	// if (op.part_id == baseMLO->exp_my_first_part_id)
	if(thread_id==0)
...
baseMLO->timer.toc(baseMLO->TIMING_ESP_DIFF2_A);
		...
		getFourierTransformsAndCtfs<MlClass>(part_id, op, sp, baseMLO, myInstance, ptrFactory, ibody,thread_id);
...
baseMLO->timer.toc(baseMLO->TIMING_ESP_DIFF2_B);
			...
			if (ipass == 0)
			{
				...
				getAllSquaredDifferencesCoarse<MlClass>(ipass, op, sp, baseMLO, myInstance, Mweight, ptrFactory, ibody, thread_id);
				CTOC(timer,"getAllSquaredDifferencesCoarse");

				CTIC(timer,"convertAllSquaredDifferencesToWeightsCoarse");
				convertAllSquaredDifferencesToWeights<MlClass>(ipass, op, sp, baseMLO, myInstance, CoarsePassWeights, FinePassClassMasks, Mweight, ptrFactory, ibody,thread_id);
...
baseMLO->timer.toc(baseMLO->TIMING_ESP_DIFF2_D);
				...
				getAllSquaredDifferencesFine<MlClass>(ipass, op, sp, baseMLO, myInstance, FinePassWeights, FinePassClassMasks, FineProjectionData, ptrFactory, ibody, bundleD2,thread_id);
				...
				convertAllSquaredDifferencesToWeights<MlClass>(ipass, op, sp, baseMLO, myInstance, FinePassWeights, FinePassClassMasks, Mweight, ptrFactory, ibody,thread_id);
...
baseMLO->timer.toc(baseMLO->TIMING_ESP_DIFF2_E);
		...
		storeWeightedSums<MlClass>(op, sp, baseMLO, myInstance, FinePassWeights, FineProjectionData, FinePassClassMasks, ptrFactory, ibody, bundleSWS,thread_id);
