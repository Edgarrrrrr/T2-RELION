...
struct OrientationMatrixHandler {
    ...
    void sync_and_store_orientation_matrix_with_reduce(
        ...
        *reinterpret_cast<float4*>(&reg_corr_div_2[0])
		...
		for (int i = 0; i < kOMNumOrientWarpTilePerWarp; i ++) {
			...
			// float4 tmp1, tmp2;
			// if ((lane_id & 0x2) == 0) {
			// 	tmp1 = *reinterpret_cast<float4*>(&reg_tex_buf_[0][i][0]);
			// 	tmp2 = *reinterpret_cast<float4*>(&reg_tex_buf_[1][i][0]);
			// }
			// else {
			// 	tmp1 = *reinterpret_cast<float4*>(&reg_tex_buf_[1][i][0]);
			// 	tmp2 = *reinterpret_cast<float4*>(&reg_tex_buf_[0][i][0]);
			// }
			...
			*reinterpret_cast<float4*>(
			...
			// *reinterpret_cast<float4*>(
			// 	get_smem_pointer(
			// 		&s_orient_mat_block_swizzle(s_orient_idx, s_img_idx1))) = tmp1;
			// *reinterpret_cast<float4*>(
			// 	get_smem_pointer(
			// 		&s_orient_mat_block_swizzle(s_orient_idx, s_img_idx2))) = tmp2;
		}
		// __syncthreads();
		// if (blockIdx.x == 0)
    ...
    void process_and_prefetch_orientation_matrix_fused_mma_tf32_sim_fp32(
		// mma
		float C[kMwarp / kMmma][kNwarp / kNmma][4], 
		SmemLayoutA s_A, 
		SmemLayoutB s_B,
			...
			for (int ii = (k/kKmma) * ((kOMNumOrientWarpTilePerWarp + kKwarp/kKmma - 1) / (kKwarp/kKmma));
				...
				*reinterpret_cast<float4*>(&reg_eulers[4])
				...
				for (int jj = 0; jj < kOMImgPerThread; jj ++) {
					...
					// reg_tex_buf_[0][ii][jj] = r2_within_bounds ? reg_xp[ii][jj] + reg_yp[ii][jj] * reg_zp[ii][jj] : 0;
					// reg_tex_buf_[1][ii][jj] = r2_within_bounds ? reg_xp[ii][jj] + reg_yp[ii][jj] * reg_zp[ii][jj] : 0;
